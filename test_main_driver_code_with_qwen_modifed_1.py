# ==============================================================================
# --- SCRIPT CONFIGURATION & AZURE PATH SETUP ---
# ==============================================================================
import os
import subprocess

# --- STEP 1: DEFINE PERMANENT AND LOCAL CACHE PATHS ---
# The master copy of models on your slow, persistent network share.
PERMANENT_HF_CACHE = "/mnt/batch/tasks/shared/LS_root/mounts/clusters/testvideoresearch/code/Users/ani31101993/VideoDeepResearch_modified/huggingface"
# A temporary directory on the VM's fast local disk. /tmp is a great choice.
LOCAL_HF_CACHE = "/tmp/local_hf_cache"

# --- STEP 2: CREATE A FUNCTION TO SYNC MODELS TO THE FAST LOCAL DISK ---
def sync_models_to_local_disk(permanent_path: str, local_path: str, model_repos: list):
    """
    Uses rsync to efficiently copy model files from the permanent network
    storage to the fast local disk. rsync is much faster than `cp` and
    will only copy missing or updated files.
    """
    print("--- Syncing models from permanent storage to fast local disk ---")
    os.makedirs(local_path, exist_ok=True)
    for repo in model_repos:
        # e.g., source becomes /mnt/.../huggingface/hub/models--Qwen--Qwen2-7B-Instruct
        # The model folder name is derived from the repo_id by replacing '/' with '--'
        model_folder_name = f"models--{repo.replace('/', '--')}"
        source_dir = os.path.join(permanent_path, 'hub', model_folder_name)
        dest_dir = os.path.join(local_path, 'hub')
        
        if not os.path.isdir(source_dir):
            print(f"⚠️ WARNING: Source model directory not found, skipping sync: {source_dir}")
            print("Please ensure the model has been downloaded to the permanent cache at least once.")
            continue
        ...
        # -a: archive mode, -h: human-readable, --progress: show progress
        rsync_command = f"rsync -ah --progress {source_dir}/ {os.path.join(dest_dir, model_folder_name)}/"
        subprocess.run(rsync_command, shell=True, check=True)
    print("--- Model sync complete. ---")


# --- STEP 3: SET HF_HOME TO THE FAST LOCAL PATH ---
# This tells Hugging Face where to LOAD models from.
os.environ['HF_HOME'] = LOCAL_HF_CACHE
print(f"✅ Hugging Face cache for this run is set to FAST LOCAL DISK: {os.environ['HF_HOME']}")
# ==============================================================================
...
# No need for snapshot_download here anymore, as we handle caching manually
...
class VideoQAManager:
    ...
    def _initialize_local_models(self):
        print("--- Loading all models FROM LOCAL DISK. This should be fast... ---")
        # We no longer need to call _download_model_if_needed because our
        # startup script `sync_models_to_local_disk` has already prepared the cache.
        
        # The .from_pretrained calls will now load from the FAST LOCAL CACHE.
...
if __name__ == "__main__":
    ...
    try:
        # --- STEP 4: CALL THE SYNC FUNCTION BEFORE INITIALIZING MODELS ---
        # This will pre-load the models onto the fast local disk
        models_to_sync = [args.llm_path, args.vlm_path, args.whisper_path]
        sync_models_to_local_disk(PERMANENT_HF_CACHE, LOCAL_HF_CACHE, models_to_sync)
